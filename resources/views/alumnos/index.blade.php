                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <select class="form-control" id="grupo" name="grupo" required>
                                    @foreach ($grupos_permitidos as $grupo)
                                        <option value="{{ $grupo->id }}">{{ $grupo->clave_grupo }}/{{$grupo->asignatura->clave}}/{{$grupo->asignatura->nombre}}</option>
                                    @endforeach
                                </select>
                            </div>
    ...
    <div class="card-body p-4">
        <div class="table-responsive">
            @if (session('grupo'))
                ...
                <h5>Asignatura: {{ session('grupo')->asignatura->nombre }}</h5>
            @endif
               
            <form id="form-post" action="{{ route('alumnos.desasignar-grupo') }}" method="POST">
                @csrf
                @if (session('grupo'))               
                <input type="hidden" name="clave_grupo" value="{{ session('grupo')->id }}">
                @endif
...
@endif
@include('alumnos.modals.confirmacion')
